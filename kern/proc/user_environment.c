...
void env_free(struct Env *e)
{
    ...
    // [4] Free All pages in the page working set
    struct WorkingSetElement * ptr;
    LIST_FOREACH(ptr, &e->page_WS_list) {
        //unmap_frame(e->env_page_directory, ptr->virtual_address);
        env_page_ws_invalidate(e,ptr->virtual_address);
  }

    //[5] Working set itself
    LIST_INIT(&(e->page_WS_list));
    // [8] Free User kernel stack
    if (e->kstack != NULL) {
    ...
    // [6] Free All page tables in the entire user virtual memory
    for (uint32 va = 0; va < USER_TOP; va += PAGE_SIZE) {
    	//cprintf("freeing block allocator \n");
        ...
        cprintf("loop 1: ptr isn't null and freed \n");
    }
//    for (uint32 va = e->UH_LIMIT+PAGE_SIZE; va < USER_HEAP_MAX; va += PAGE_SIZE) {
//    	uint32 *ptr_page_table=NULL;
//    	get_page_table(e->env_page_directory, va,&ptr_page_table);
//        if(ptr_page_table==NULL){
//        	continue;
//        }
//        kfree((void*)ptr_page_table);
//        cprintf("loop 2: ptr isn't null and freed \n");
//        pt_clear_page_table_entry(e->env_page_directory, (uint32)va);
//        pd_clear_page_dir_entry(e->env_page_directory, (uint32)va);
//    }



    // [7] Free Directory table
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
    ...
    uint32 *returnable = kmalloc(KERNEL_STACK_SIZE);
    if(returnable==NULL)
    panic("Unavailable Space for user stack!!!");

    pt_set_page_permissions(ptr_user_page_directory, (uint32)returnable, 0, PERM_PRESENT);
...
//	kfree((void *)va);
//	pt_set_page_permissions(e->env_page_directory, (uint32)va, PERM_PRESENT,0 );
//	}
