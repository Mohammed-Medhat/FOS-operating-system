...
void sched_insert_ready(struct Env* env)
{
	...
	assert(env != NULL);
	{
		//cprintf("\nInserting %d into ready queue 0\n", env->env_id);
		env->env_status = ENV_READY ;
		env->number_of_ticks = timer_ticks();
...
void env_set_priority(int envID, int priority)
{
	...
	if(check==0){
	    ...
	    if(proc->env_status==ENV_READY) {
	    	//sched_remove_ready(proc); //comment?
...
void wait_helper(struct Env_Queue* queue){
...
void signal_helper(struct Env_Queue* queue){
	/* remove a process P from s.queue */
	struct Env* semEnv=dequeue(queue);
	/* place process P on ready list */
    acquire_spinlock(&ProcessQueues.qlock);
    sched_insert_ready(semEnv);
    release_spinlock(&ProcessQueues.qlock);
}
