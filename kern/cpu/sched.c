...
void clock_interrupt_handler(struct Trapframe* tf)
{
  if (isSchedMethodPRIRR())
  {
	        ...
	        for (int i=0;i<num_of_ready_queues;i++)
	        {
	        	if (i==0) continue; //no promotion for priority 0
	            if (!LIST_EMPTY(&(ProcessQueues.env_ready_queues[i])))
	            {
	                //cprintf("Iterating over READY queue #%d...\n",i);
	                struct Env *ptr_env;
	                //ptr_env->env_tf=tf; //s7 wla eftkasa hndm 3leha?
	                LIST_FOREACH(ptr_env, &(ProcessQueues.env_ready_queues[i]))
	                {
	                    int64 ticksPassed=timer_ticks()-ptr_env->number_of_ticks;
	                    if(ticksPassed>(int64)starvThreshglobal && ptr_env->priority>0){
	                        remove_from_queue(&ProcessQueues.env_ready_queues[i],ptr_env);
	                        release_spinlock(&ProcessQueues.qlock);
	                        env_set_priority(ptr_env->env_id,ptr_env->priority-1);
	                        acquire_spinlock(&ProcessQueues.qlock);
	                        //cprintf("Process %d promoted to priority %d due to starvation.\n", ptr_env->env_id, ptr_env->priority);
	                    }
	                }
	            }
	        }release_spinlock(&ProcessQueues.qlock);
