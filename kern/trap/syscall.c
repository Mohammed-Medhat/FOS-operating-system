...
void sys_wait_helper(struct Env_Queue* queue){
	wait_helper(queue);
}

void sys_signal_helper(struct Env_Queue* queue){
	signal_helper(queue);
}
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_allocate_user_mem:
		...
		else if(a1 < USER_HEAP_START || a1 > USER_HEAP_MAX){
			env_exit();
		}
	...
	case NSYSCALLS:
		return 	-E_INVAL;
		break;
	case SYS_wait_helper:
		sys_wait_helper((struct Env_Queue*)a1);
		return 0;

	case SYS_signal_helper:
		sys_signal_helper((struct Env_Queue*)a1);
		return 0;
