...
int execute_command(char *command_string)
{
	...
	else if (ret == CMD_INV_NUM_ARGS)
	{
		...
		cprintf("Number of found commands: %d\n", numOfFoundCmds);
		if (numOfFoundCmds != 1)
		{
...
int process_command(int number_of_arguments, char** arguments)
{
	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++)
	{
		...
		{
			if( commands[i].num_of_args > 0 && commands[i].num_of_args != (number_of_arguments -1))
			{
	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++){
					int cntr=0;
					char* newpos=commands[i].name;
					for(int j=0;j<strlen(arguments[0]);j++){
						newpos=strchr(newpos,arguments[0][j]);
						if(newpos!=NULL){
							newpos++;
							cntr++;
						}
					...
					if(cntr==strlen(arguments[0])){
						LIST_INSERT_TAIL(&foundCommands,&commands[i]);
					}
		}
	if (LIST_SIZE(&foundCommands) >= 1) {
		return CMD_MATCHED;
	}
